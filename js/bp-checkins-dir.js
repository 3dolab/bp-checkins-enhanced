jQuery(document).ready(function($){
	...
	$('#bpci-show-on-map').live( 'click', function(){
		...
		if( buttonAction == 'show' ) {
			$("#bpci-map").gmap3({
				...
				overlay: { 
					latLng: position, 
					data: '<div class="bpci-avatar"><s></s><i></i><span>' + $("#whats-new-avatar").html() + '</span></div>',
					options: {
	...
	$('#bpci-place-show-on-map').click(function(){
		...
		bpci_search_position( address, '#bpci-places-map', '#bpci-place-address', '#bpci-place-lat', '#bpci-place-lng', false, avatar, 'places' );
		
		return false;
	});
	
	function bpci_search_position( address, map, addressField, latField, lngField, cookie, avatar, scope ) {
		...
		geocoder.geocode( { 'address': address}, function(results, status) {
		    ...
		    if (status == google.maps.GeocoderStatus.OK) {
				...
				if(scope == 'places') {
					if( ( $('#bpci-place-address').attr('readonly') && $('#bpci-place-address').attr('readonly')=='readonly') || ( $('#bpci-place-lat').val() && $('#bpci-place-lng').val() ) )
						toClear = 'marker';
					$(map).gmap3({
						marker: { 
							latLng: position,
					...
					if( ( $('#bpci-address').attr('readonly') && $('#bpci-address').attr('readonly')=='readonly') || ( $('#bpci-lat').val() && $('#bpci-lng').val() ) )
						toClear = 'overlay';
					$(map).gmap3({
						overlay: { 
							latLng: position, 
							data: '<div class="bpci-avatar"><s></s><i></i><span>' + avatar + '</span></div>',
							options: {
	...
	function bpci_is_on_map(lat, lng, data){
		var onmap = -1;
		for(var i=0; i < arrayMarkers.length ; i++){
			if(arrayMarkers[i].lat == lat && arrayMarkers[i].lng == lng && arrayMarkers[i].data == data)
				onmap=i;
		}
		return onmap;
	}
  ...
  function add($this, i, lat, lng, data, pin, scope){
    if(scope=='places')
		$this.gmap3({
			marker: { 
				latLng:[lat, lng], 
		...
		$this.gmap3({
  		...
  		$('#bpci-map_container').html('<div id="#bpci-map"></div>');
